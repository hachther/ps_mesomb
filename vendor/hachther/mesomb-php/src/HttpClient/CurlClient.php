...
use MeSomb\Util\CaseInsensitiveArray;
...
class CurlClient
{
     ...
     * @param null|RandomGenerator $randomGenerator
     */
     ...
     *   <li>\MeSomb\Util\CaseInsensitiveArray $rheaders The response headers</li>
    ...
    private function constructRequest($method, $absUrl, $headers, $params, $hasFile = false) {
        ...
        if (('post' === $method) && (MeSomb::$maxNetworkRetries > 0)) {
...
//        }
        ...
        // force IPv4 resolving as MeSomb's API servers are only accessible over
        // IPv4 (see. https://github.com/hachther/mesomb-php-client/issues).
     ...
     *
     * @throws MeSomb\Exception\ServerException
     */
    private function handleCurlError($url, $errno, $message, $numRetries)
    {
        switch ($errno) {
            ...
            case \CURLE_OPERATION_TIMEOUTED:
                $msg = "Could not connect to MeSomb ({$url}).  Please check your "
                    . 'internet connection and try again.  If this problem persists, '
                    . "you should check MeSomb's service,";

                break;

            case \CURLE_SSL_CACERT:
            case \CURLE_SSL_PEER_CERTIFICATE:
                $msg = "Could not verify MeSomb's SSL certificate.  Please make sure "
            ...
            default:
                $msg = 'Unexpected error communicating with MeSomb.  '
                    . 'If this problem persists,';
        }
        $msg .= ' let us know at support@mesomb.atlassian.net.';
    ...
    {
        if ($numRetries >= MeSomb::getMaxNetworkRetries()) {
            return false;
        }
        ...
        if (isset($rheaders['mesomb-should-retry'])) {
            if ('false' === $rheaders['mesomb-should-retry']) {
                return false;
            }
            if ('true' === $rheaders['mesomb-should-retry']) {
                return true;
            }
        }
        ...
        //
        // Note that we expect the mesomb-should-retry header to be false
    ...
    private function sleepTime($numRetries, $rheaders)
    {
        ...
        $sleepSeconds = \min(
            MeSomb::getInitialNetworkRetryDelay() * 1.0 * 2 ** ($numRetries - 1),
            MeSomb::getMaxNetworkRetryDelay()
        );
        ...
        $sleepSeconds = \max(MeSomb::getInitialNetworkRetryDelay(), $sleepSeconds);

        // And never sleep less than the time the API asks us to wait, assuming it's a reasonable ask.
        $retryAfter = isset($rheaders['retry-after']) ? (float) ($rheaders['retry-after']) : 0.0;
        if (\floor($retryAfter) === $retryAfter && $retryAfter <= MeSomb::getMaxRetryAfter()) {
    ...
    private function canSafelyUseHttp2()
    {
        ...
        // (cf. https://github.com/curl/curl/issues/2416), which MeSomb use.
